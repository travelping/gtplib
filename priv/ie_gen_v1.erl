#!/usr/bin/env escript
%% -*- erlang -*-
%%! -smp enable

-mode(compile).

-define(es, erl_syntax).
-define(V1_TAG, <<"%% -include(\"gtp_packet_v1_gen.hrl\").">>).
-define(V2_TAG, <<"%% -include(\"gtp_packet_v2_gen.hrl\").">>).

ies() ->
    [{1, "Cause", 1,
      [{"Value", 8, {enum, [{0, "Request IMSI"},
                            {1, "Request IMEI"},
                            {2, "Request IMSI and IMEI"},
                            {3, "No identity needed"},
                            {4, "Request MS Refuses"},
                            {5, "Request MS is not GPRS Responding"},
                            {6, "Reactivation Requested"},
                            {7, "PDP address inactivity timer expires"},
                            {8, "Network Failure"},
                            {9, "QoS parameter mismatch"},
                            {59, "GTPPrime System failure"},
                            {60, "GTPPrime The transmit buffers are becoming full"},
                            {61, "GTPPrime The receive buffers are becoming full"},
                            {62, "GTPPrime Another node is about to go down"},
                            {63, "GTPPrime This node is about to go down"},
                            {128, "Request accepted"},
                            {129, "New PDP type due to network preference"},
                            {130, "New PDP type due to single address bearer only"},
                            {177, "CDR Decoding Error"},
                            {192, "Non-existent"},
                            {193, "Invalid message format"},
                            {194, "IMSI/IMEI not known"},
                            {195, "MS is GPRS Detached"},
                            {196, "Reject MS is not GPRS Responding"},
                            {197, "Reject MS Refuses"},
                            {198, "Version not supported"},
                            {199, "No resources available"},
                            {200, "Service not supported"},
                            {201, "Mandatory IE incorrect"},
                            {202, "Mandatory IE missing"},
                            {203, "Optional IE incorrect"},
                            {204, "System failure"},
                            {205, "Roaming restriction"},
                            {206, "P-TMSI Signature mismatch"},
                            {207, "GPRS connection suspended"},
                            {208, "Authentication failure"},
                            {209, "User authentication failed"},
                            {210, "Context not found"},
                            {211, "All dynamic PDP addresses are occupied"},
                            {212, "No memory is available"},
                            {213, "Relocation failure"},
                            {214, "Unknown mandatory extension header"},
                            {215, "Semantic error in the TFT operation"},
                            {216, "Syntactic error in the TFT operation"},
                            {217, "Semantic errors in packet filter"},
                            {218, "Syntactic errors in packet filter"},
                            {219, "Missing or unknown APN"},
                            {220, "Unknown PDP address or PDP type"},
                            {221, "PDP context without TFT already activated"},
                            {222, "APN access denied no subscription"},
                            {223, "APN Restriction type incompatibility with currently active PDP Contexts"},
                            {224, "MS MBMS Capabilities Insufficient"},
                            {225, "Invalid Correlation-ID"},
                            {226, "MBMS Bearer Context Superseded"},
                            {227, "Bearer Control Mode violation"},
                            {228, "Collision with network initiated request"},
                            {229, "APN Congestion"},
                            {230, "Bearer handling not supported"},
                            {231, "Target access restricted for the subscriber"},
                            {252, "Request related to possibly duplicated packets already fulfilled"},
                            {253, "Request already fulfilled"},
                            {254, "Sequence numbers of released/cancelled packets IE incorrect"},
                            {255, "Request not fulfilled"}]}}
      ]},
     {2, "International Mobile Subscriber Identity", 8,
      [{"IMSI", 64, {type, imsi}}]},
     {3, "Routeing Area Identity", 6, v1_rai},
     {4, "Temporary Logical Link Identity", 4,
      [{"TLLI", 4, bytes}]},
     {5, "Packet TMSI", 4,
      [{"P-TMSI", 4, bytes}]},
     {8, "Reordering Required", 1,
      [{'1', 7},
       {"required", 1, {enum, [no, yes]}}]},
     {9, "Authentication Triplet", 28,
      [{"RAND", 16, bytes},
       {"SRES", 4, bytes},
       {"Kc", 8, bytes}]},
     {11, "MAP Cause", 1,
      [{"Value", 1, bytes}]},
     {12, "P-TMSI Signature", 3,
      [{"Value", 3, bytes}]},
     {13, "MS Validated", 1,
      [{'1', 7},
       {"validated", 1, {enum, [no, yes]}}]},
     {14, "Recovery", 1,
      [{"Restart counter", 8, integer}]},
     {15, "Selection Mode", 1,
      [{'1', 6},
       {"Mode", 2, integer}]},
     {16, "Tunnel Endpoint Identifier Data I", 4,
      [{"TEI", 32, integer}]},
     {17, "Tunnel Endpoint Identifier Control Plane", 4,
      [{"TEI", 32, integer}]},
     {18, "Tunnel Endpoint Identifier Data II", 5,
      [{'_', 4},
       {"NSAPI", 4, integer},
       {"TEI", 32, integer}]},
     {19, "Teardown Ind", 1,
      [{'1', 7},
       {"Value", 1, integer}]},
     {20, "NSAPI", 1,
      [{'_', 4},
       {"NSAPI", 4, integer}]},
     {21, "RANAP Cause", 1,
      [{"Value", 8, integer}]},
     {22, "RAB Context", 9,
      [{'_', 4},
       {"NSAPI", 4, integer},
       {"DL GTP-U Sequence Number", 16, integer},
       {"UL GTP-U Sequence Number", 16, integer},
       {"DL PDCP Sequence Number", 16, integer},
       {"UL PDCP Sequence Number", 16, integer}]},
     {23, "Radio Priority SMS", 1,
      [{'_', 5},
       {"Value", 3, integer}]},
     {24, "Radio Priority", 1,
      [{"NSAPI", 4, integer},
       {'_', 1},
       {"Value", 3, integer}]},
     {25, "Packet Flow Id", 2,
      [{'_', 4},
       {"NSAPI", 4, integer},
       {"Value", 8, integer}]},
     {26, "Charging Characteristics", 2,
      [{"Value", 2, bytes}]},
     {27, "Trace Reference", 2,
      [{"Value", 16, integer}]},
     {28, "Trace Type", 2,
      [{"Value", 16, integer}]},
     {29, "MS Not Reachable Reason", 1,
      [{"Value", 8, integer}]},
     {126, "Packet Transfer Command", 1,
      [{"Command", 8, {enum, [{1, "Send Data Record Packet"},
                              {2, "Send possibly duplicated Data Record Packet"},
                              {3, "Cancel Data Record Packet"},
                              {4, "Release Data Record Packet"}]}}]},
     {127, "Charging ID", 4,
      [{id, 4, bytes}]},
     {128, "End User Address", '_',
      [{'1', 4},
       {"PDP Type Organization", 4, integer},
       {"PDP Type Number", 8, integer},
       {"PDP Address", 0, binary}]},
     {129, "MM Context GSM", '_',
      [{'1', 4},
       {"CKSN", 4, integer},
       {1, 2},
       {"No of Vectors", 3, integer},
       {"Used Cipher", 3, integer},
       {"Kc", 8, bytes},
       {"Tripple", 8, {array, "No of Vectors"}},
       {"DRX parameter", 2, bytes},
       {"MS Network Capability Length", 8, integer},
       {"MS Network Capability", 1, {array, "MS Network Capability Length"}},
       {"Container Length", 16, integer},
       {"Container", 1, {array, "Container Length"}},
       {'_', 0}
      ]},
     {129, "MM Context UMTS", '_',
      [{'1', 4},
       {"KSI", 4, integer},
       {2, 2},
       {"No of Vectors", 3, integer},
       {'1', 3},
       {"CK", 16, bytes},
       {"IK", 16, bytes},
       {"Quintuplet Length", 16, integer},
       {"Quintuplet", 1, {array, "Quintuplet Length"}},
       {"DRX parameter", 2, bytes},
       {"MS Network Capability Length", 8, integer},
       {"MS Network Capability", 1, {array, "MS Network Capability Length"}},
       {"Container Length", 16, integer},
       {"Container", 1, {array, "Container Length"}},
       {'_', 0}
      ]},
     {129, "MM Context GSM and UMTS", '_',
      [{'1', 4},
       {"CKSN", 4, integer},
       {3, 2},
       {"No of Vectors", 3, integer},
       {"Used Cipher", 3, integer},
       {"Kc", 8, bytes},
       {"Quintuplet Length", 16, integer},
       {"Quintuplet", 1, {array, "Quintuplet Length"}},
       {"DRX parameter", 2, bytes},
       {"MS Network Capability Length", 8, integer},
       {"MS Network Capability", 1, {array, "MS Network Capability Length"}},
       {"Container Length", 16, integer},
       {"Container", 1, {array, "Container Length"}},
       {'_', 0}
      ]},
     {129, "MM Context UMTS and Used Cipher", '_',
      [{'1', 4},
       {"KSI", 4, integer},
       {0, 2},
       {"No of Vectors", 3, integer},
       {"Used Cipher", 3, integer},
       {"CK", 16, bytes},
       {"IK", 16, bytes},
       {"Quintuplet Length", 16, integer},
       {"Quintuplet", 1, {array, "Quintuplet Length"}},
       {"DRX parameter", 2, bytes},
       {"MS Network Capability Length", 8, integer},
       {"MS Network Capability", 1, {array, "MS Network Capability Length"}},
       {"Container Length", 16, integer},
       {"Container", 1, {array, "Container Length"}},
       {'_', 0}
      ]},
     {130, "PDP Context", '_',
      []},
     {131, "Access Point Name", '_',
      [{"APN", 0, {type, fqdn}}]},
     {132, "Protocol Configuration Options", '_',
      %% TODO,  The content and the coding of the Protocol Configuration are
      %% defined in octet 3-z of the Protocol Configuration Options in
      %% subclause 10.5.6.3 of 3GPP TS 24.008 [5]. Please refer to subclause 10.5.6.3 of
      %% 3GPP TS 24.008 [5] for the maximum length of Protocol Configuration Options.
      [{"Config", protocol_config_opts}]},
     {133, "GSN Address", '_',
      [{"Address", 0, binary}]},
     {134, "MS International PSTN/ISDN Number", '_',
      [{"MSISDN", isdn_address_string}]},
     {135, "Quality of Service Profile", '_',
      [{"Priority", 8, integer},
       {"Data", 0, binary}]},
     {136, "Authentication Quintuplet", '_',
      []},
     {137, "Traffic Flow Template", '_',
      []},
     {138, "Target Identification", '_',
      []},
     {139, "UTRAN Transparent Container", '_',
      []},
     {140, "RAB Setup Information", '_',
      []},
     {141, "Extension Header Type List", '_',
      []},
     {142, "Trigger Id", '_',
      []},
     {143, "OMC Identity", '_',
      []},
     {144, "RAN Transparent Container", '_',
      []},
     {145, "PDP Context Prioritization", '_',
      []},
     {146, "Additional RAB Setup Information", '_',
      []},
     {147, "SGSN Number", '_',
      []},
     {148, "Common Flags", '_',
      [{"Flags", {flags, ["Dual Address Bearer Flag",
                          "Upgrade QoS Supported",
                          "NRSN",
                          "No QoS negotiation",
                          "MBMS Counting Information",
                          "RAN Procedures Ready",
                          "MBMS Service Type",
                          "Prohibit Payload Compression"]}}]},
     {149, "APN Restriction", '_',
      [{"Restriction Type Value", 8, integer}]},
     {150, "Radio Priority LCS", '_',
      []},
     {151, "RAT Type", '_',
      [{"RAT Type", 8, integer},
       {'_', 0}]},
     {152, "User Location Information", v1_uli},
     {153, "MS Time Zone", '_',
      [{"TimeZone", 8, integer},
       {'_', 6},
       {"DST", 2, integer},
       {'_', 0}]},
     {154, "IMEI", '_',
      [{"IMEI", 64, {type, imei}},
       {'_', 0}]},
     {155, "CAMEL Charging Information Container", '_',
      []},
     {156, "MBMS UE Context", '_',
      []},
     {157, "Temporary Mobile Group Identity", '_',
      []},
     {158, "RIM Routing Address", '_',
      []},
     {159, "MBMS Protocol Configuration Options", '_',
      []},
     {160, "MBMS Service Area", '_',
      []},
     {161, "Source RNC PDCP context info", '_',
      []},
     {162, "Additional Trace Info", '_',
      []},
     {163, "Hop Counter", '_',
      []},
     {164, "Selected PLMN ID", '_',
      []},
     {165, "MBMS Session Identifier", '_',
      []},
     {166, "MBMS 2G/3G Indicator", '_',
      []},
     {167, "Enhanced NSAPI", '_',
      []},
     {168, "MBMS Session Duration", '_',
      []},
     {169, "Additional MBMS Trace Info", '_',
      []},
     {170, "MBMS Session Repetition Number", '_',
      []},
     {171, "MBMS Time To Data Transfer", '_',
      []},
     {173, "BSS Container", '_',
      []},
     {174, "Cell Identification", '_',
      []},
     {175, "PDU Numbers", '_',
      []},
     {176, "BSSGP Cause", '_',
      []},
     {177, "Required MBMS bearer capabilities", '_',
      []},
     {178, "RIM Routing Address Discriminator", '_',
      []},
     {179, "List of set-up PFCs", '_',
      []},
     {180, "PS Handover XID Parameters", '_',
      []},
     {181, "MS Info Change Reporting Action", '_',
      [{"Action", 8, {enum, [{0, "Stop Reporting"},
                             {1, "Start Reporting CGI/SAI"},
                             {2, "Start Reporting RAI"}]}}]},
     {182, "Direct Tunnel Flags", '_',
      []},
     {183, "Correlation-ID", '_',
      []},
     {184, "Bearer Control Mode", '_',
      []},
     {185, "MBMS Flow Identifier", '_',
      []},
     {186, "MBMS IP Multicast Distribution", '_',
      []},
     {187, "MBMS Distribution Acknowledgement", '_',
      []},
     {188, "Reliable INTER RAT HANDOVER INFO", '_',
      []},
     {189, "RFSP Index", '_',
      []},
     {190, "Fully Qualified Domain Name", '_',
      [{"FQDN", 0, {type, fqdn}}]},
     {191, "Evolved Allocation/Retention Priority I", '_',
      [{'_', 1},
       {"PCI", 1, integer},
       {"PL", 4, integer},
       {'_', 1},
       {"PVI", 1, integer}]},
     {192, "Evolved Allocation/Retention Priority II", '_',
      []},
     {193, "Extended Common Flags", '_',
      [{"Flags", {flags, ["UASI",
                          "BDWI",
                          "PCRI",
                          "VB",
                          "RetLoc",
                          "CPSR",
                          "CCRSI",
                          "Unauthenticated IMSI"]}}]},
     {194, "User CSG Information", '_',
      []},
     {195, "CSG Information Reporting Action", '_',
      []},
     {196, "CSG ID", '_',
      []},
     {197, "CSG Membership Indication", '_',
      []},
     {198, "Aggregate Maximum Bit Rate", '_',
      [{"Uplink", 32, integer},
       {"Downlink", 32, integer}
      ]},
     {199, "UE Network Capability", '_',
      []},
     {200, "UE-AMBR", '_',
      []},
     {201, "APN-AMBR with NSAPI", '_',
      []},
     {202, "GGSN Back-Off Time", '_',
      []},
     {203, "Signalling Priority Indication", '_',
      []},
     {204, "Signalling Priority Indication with NSAPI", '_',
      []},
     {205, "Higher bitrates than 16 Mbps flag", '_',
      []},
     {207, "Additional MM context for SRVCC", '_',
      []},
     {208, "Additional flags for SRVCC", '_',
      []},
     {209, "STN-SR", '_',
      []},
     {210, "C-MSISDN", '_',
      []},
     {211, "Extended RANAP Cause", '_',
      []},
     {212, "eNodeB ID", '_',
      []},
     {213, "Selection Mode with NSAPI", '_',
      []},
     {214, "ULI Timestamp", '_',
      []},
     {215, "Local Home Network ID with NSAPI", '_',
      []},
     {216, "CN Operator Selection Entity", '_',
      []},
     {249, "Sequence Numbers of Released Packets", '_',
      [{"Sequence Numbers", array_of_seq_no}]},
     {250, "Sequence Numbers of Cancelled Packets", '_',
      [{"Sequence Numbers", array_of_seq_no}]},
     {251, "Charging Gateway Address", '_',
      [{"Address", 0, binary}]},
     {252, "Data Record Packet", data_record_packet},
     {253, "Requests Responded", '_',
      [{"Sequence Numbers", array_of_seq_no}]},
     {254, "Address of Recommended Node", '_',
      [{"Address", 0, binary}]},
     {255, "Private Extension", '_',
      [{"Enterprise Id", 16, integer},
       {"Value", 0, binary}]}
    ].

msgs() ->
    [{1, "Echo Request"},
     {2, "Echo Response"},
     {3, "Version Not Supported"},
     {4, "Node Alive Request"},
     {5, "Node Alive Response"},
     {6, "Redirection Request"},
     {7, "Redirection Response"},
     {16, "Create PDP Context Request"},
     {17, "Create PDP Context Response"},
     {18, "Update PDP Context Request"},
     {19, "Update PDP Context Response"},
     {20, "Delete PDP Context Request"},
     {21, "Delete PDP Context Response"},
     {22, "Initiate PDP Context Activation Request"},
     {23, "Initiate PDP Context Activation Response"},
     {26, "Error Indication"},
     {27, "PDU Notification Request"},
     {28, "PDU Notification Response"},
     {29, "PDU Notification Reject Request"},
     {30, "PDU Notification Reject Response"},
     {31, "Supported Extension Headers Notification"},
     {32, "Send Routeing Information for GPRS Request"},
     {33, "Send Routeing Information for GPRS Response"},
     {34, "Failure Report Request"},
     {35, "Failure Report Response"},
     {36, "Note MS GPRS Present Request"},
     {37, "Note MS GPRS Present Response"},
     {48, "Identification Request"},
     {49, "Identification Response"},
     {50, "SGSN Context Request"},
     {51, "SGSN Context Response"},
     {52, "SGSN Context Acknowledge"},
     {53, "Forward Relocation Request"},
     {54, "Forward Relocation Response"},
     {55, "Forward Relocation Complete"},
     {56, "Relocation Cancel Request"},
     {57, "Relocation Cancel Response"},
     {58, "Forward SRNS Context"},
     {59, "Forward Relocation Complete Acknowledge"},
     {60, "Forward SRNS Context Acknowledge"},
     {70, "RAN Information Relay"},
     {96, "MBMS Notification Request"},
     {97, "MBMS Notification Response"},
     {98, "MBMS Notification Reject Request"},
     {99, "MBMS Notification Reject Response"},
     {100, "Create MBMS Context Request"},
     {101, "Create MBMS Context Response"},
     {102, "Update MBMS Context Request"},
     {103, "Update MBMS Context Response"},
     {104, "Delete MBMS Context Request"},
     {105, "Delete MBMS Context Response"},
     {112, "MBMS Registration Request"},
     {113, "MBMS Registration Response"},
     {114, "MBMS De-Registration Request"},
     {115, "MBMS De-Registration Response"},
     {116, "MBMS Session Start Request"},
     {117, "MBMS Session Start Response"},
     {118, "MBMS Session Stop Request"},
     {119, "MBMS Session Stop Response"},
     {120, "MBMS Session Update Request"},
     {121, "MBMS Session Update Response"},
     {128, "MS Info Change Notification Request"},
     {129, "MS Info Change Notification Response"},
     {240, "Data Record Transfer Request"},
     {241, "Data Record Transfer Response"},
     {254, "End Marker"},
     {255, "G-PDU"}
    ].

gen_record_def([]) ->
    [];
gen_record_def([{Value, _} | More]) when is_integer(Value); is_atom(Value) ->
    gen_record_def(More);
gen_record_def([{Name, {flags, _}} | More]) ->
    Form = ?es:record_field(es_atom(s2a(Name)), ?es:map_expr(none, [])),
    [Form | gen_record_def(More)];
gen_record_def([{Name, _, {enum, [{_,H}|_]}} | More]) ->
    Form = ?es:record_field(es_atom(s2a(Name)), es_atom(s2a(H))),
    [Form | gen_record_def(More)];
gen_record_def([{Name, _, {enum, [H|_]}} | More]) ->
    Form = ?es:record_field(es_atom(s2a(Name)), es_atom(s2a(H))),
    [Form | gen_record_def(More)];
gen_record_def([{Name, _, integer} | More]) ->
    Form =
        ?es:typed_record_field(
           ?es:record_field(es_atom(s2a(Name)), ?es:integer(0)),
           ?es:type_application(?es:atom(non_neg_integer), [])),
    [Form | gen_record_def(More)];
gen_record_def([{Name, boolean} | More]) ->
    Form =
        ?es:typed_record_field(
           ?es:record_field(es_atom(s2a(Name)), ?es:atom(false)),
           ?es:type_application(?es:atom(boolean), [])),
    [Form | gen_record_def(More)];
gen_record_def([{Name, Size, bits} | More]) ->
    Form =
        ?es:record_field(es_atom(s2a(Name)),
                         ?es:binary(
                            [es:binary_field(?es:integer(0), ?es:integer(Size), [])])),
    [Form | gen_record_def(More)];
gen_record_def([{Name, Size, bytes} | More]) ->
    Form =
        ?es:record_field(
           es_atom(s2a(Name)),
           ?es:binary(
              [?es:binary_field(
                  ?es:integer(0),
                  ?es:integer(Size),
                  [?es:size_qualifier(?es:atom(unit), ?es:integer(8))])
              ])
          ),
    [Form | gen_record_def(More)];
gen_record_def([{Name, _, binary} | More]) ->
    Form = ?es:record_field(es_atom(s2a(Name)), ?es:binary([])),
    [Form | gen_record_def(More)];
gen_record_def([{Name, _, length_binary} | More]) ->
    Form = ?es:record_field(es_atom(s2a(Name)), ?es:binary([])),
    [Form | gen_record_def(More)];
gen_record_def([{Name, _, {array, _}} | More]) ->
    Form = ?es:record_field(es_atom(s2a(Name)), ?es:list([])),
    [Form | gen_record_def(More)];
gen_record_def([Tuple | More]) ->
    Form = ?es:record_field(es_atom(s2a(element(1, Tuple)))),
    [Form | gen_record_def(More)].

decoder_header_match_split({_Name, _Len, {array, _Multi}}) ->
    false;
decoder_header_match_split(_) ->
    true.

gen_decoder_header_match([]) ->
    [];
gen_decoder_header_match([{'_', 0} | More]) ->
    %% ["_/binary"];
    Form =
        ?es:binary_field(
           ?es:underscore(),
           [?es:atom("binary")]),
    [Form | gen_decoder_header_match(More)];
gen_decoder_header_match([{'_', Size} | More]) ->
    %% [io_lib:format("_:~w", [Size])];
    Form =
        ?es:binary_field(
           ?es:underscore(),
           ?es:integer(Size),
           []),
    [Form | gen_decoder_header_match(More)];
gen_decoder_header_match([{'1', Size} | More]) ->
    %% [io_lib:format("_:~w", [Size])];
    Form =
        ?es:binary_field(
           ?es:underscore(),
           ?es:integer(Size),
           []),
    [Form | gen_decoder_header_match(More)];
gen_decoder_header_match([{Value, Size} | More]) when is_integer(Value) ->
    %% [io_lib:format("~w:~w", [Value, Size])];
    Form =
        ?es:binary_field(
           ?es:integer(Value),
           ?es:integer(Size),
           []),
    [Form | gen_decoder_header_match(More)];
gen_decoder_header_match([{Value, Size} | More]) when is_atom(Value) ->
    %% [io_lib:format("~w:~w", [Value, Size])];
    Form =
        ?es:binary_field(
           ?es:atom(Value),
           ?es:integer(Size),
           []),
    [Form | gen_decoder_header_match(More)];
gen_decoder_header_match([{Name, {flags, _}} | More]) ->
    %% [io_lib:format("M_~s/binary", [s2a(Name)])];
    Form =
        ?es:binary_field(
           ?es:variable(s2a("M_~s", Name)),
           [?es:atom("binary")]),
    [Form | gen_decoder_header_match(More)];
gen_decoder_header_match([{Name, boolean} | More]) ->
    %% [io_lib:format("M_~s:1", [s2a(Name)])];
    Form =
        ?es:binary_field(
           ?es:variable(s2a("M_~s", Name)),
           ?es:integer(1),
           []),
    [Form | gen_decoder_header_match(More)];
gen_decoder_header_match([{Name, Size, {enum, _Enum}} | More]) ->
    %% [io_lib:format("M_~s:~w/integer", [s2a(Name), Size])];
    Form =
        ?es:binary_field(
           ?es:variable(s2a("M_~s", Name)),
           ?es:integer(Size),
           [?es:atom("integer")]),
    [Form | gen_decoder_header_match(More)];
gen_decoder_header_match([{Name, _Fun} | More]) ->
    %% [io_lib:format("M_~s/binary", [s2a(Name)])];
    Form =
        ?es:binary_field(
           ?es:variable(s2a("M_~s", Name)),
           [?es:atom("binary")]),
    [Form | gen_decoder_header_match(More)];
gen_decoder_header_match([{Name, _Len, {array, Multi}} | _]) when is_list(Multi) ->
    %% {stop, [io_lib:format("M_~s_Rest/binary", [s2a(Name)])]};
    Form =
        ?es:binary_field(
           ?es:variable(s2a("M_~s_Rest", Name)),
           [?es:atom("binary")]),
    [Form];
gen_decoder_header_match([{Name, Len, {array, _Multi}} | _]) ->
    %% {stop, [io_lib:format("M_~s_len:~w/integer, M_~s_Rest/binary", [s2a(Name), Len, s2a(Name)])]};
    Form1 =
        ?es:binary_field(
           ?es:variable(s2a("M_~s_len", Name)),
           ?es:integer(Len),
           [?es:atom("integer")]),
    Form2 =
        ?es:binary_field(
           ?es:variable(s2a("M_~s_Rest", Name)),
           [?es:atom("binary")]),
    [Form1, Form2];
gen_decoder_header_match([{Name, Len, length_binary} | More]) ->
    %% [io_lib:format("M_~s_len:~w/integer, M_~s:M_~s_len/bytes", [s2a(Name), Len, s2a(Name), s2a(Name)])];
    Form1 =
        ?es:binary_field(
           ?es:variable(s2a("M_~s_len", Name)),
           ?es:integer(Len),
           [?es:atom("integer")]),
    Form2 =
        ?es:binary_field(
           ?es:variable(s2a("M_~s", Name)),
           ?es:variable(s2a("M_~s_len", Name)),
           [?es:atom("bytes")]),
    [Form1, Form2 | gen_decoder_header_match(More)];
gen_decoder_header_match([{Name, 0, {type, _TypeName}} | More]) ->
    %% [io_lib:format("M_~s/binary", [s2a(Name)])];
    Form =
        ?es:binary_field(
           ?es:variable(s2a("M_~s", Name)),
           [?es:atom("binary")]),
    [Form | gen_decoder_header_match(More)];
gen_decoder_header_match([{Name, 0, Type} | More]) ->
    %% [io_lib:format("M_~s/~w", [s2a(Name), Type])];
    Form =
        ?es:binary_field(
           ?es:variable(s2a("M_~s", Name)),
           [?es:atom(Type)]),
    [Form | gen_decoder_header_match(More)];
gen_decoder_header_match([{Name, Size, {type, _TypeName}} | More]) ->
    %% [io_lib:format("M_~s:~w/bits", [s2a(Name), Size])];
    Form =
        ?es:binary_field(
           ?es:variable(s2a("M_~s", Name)),
           ?es:integer(Size),
           [?es:atom("bits")]),
    [Form | gen_decoder_header_match(More)];
gen_decoder_header_match([{Name, Size, Type} | More]) ->
    %% [io_lib:format("M_~s:~w/~s", [s2a(Name), Size, Type])].
    Form =
        ?es:binary_field(
           ?es:variable(s2a("M_~s", Name)),
           ?es:integer(Size),
           [?es:atom(Type)]),
    [Form | gen_decoder_header_match(More)].

gen_decoder_record_assign([]) ->
    [];
gen_decoder_record_assign([{Value, _} | More]) when is_integer(Value); is_atom(Value) ->
    %% [];
    gen_decoder_record_assign(More);
gen_decoder_record_assign([{Name, {flags, Flags0}} | More]) ->
    %% Flags = [s2f(X) || X <- Flags0],
    %% [io_lib:format("~s = decode_flags(M_~s, ~p)", [s2a(Name), s2a(Name), Flags])];
    Flags = [es_atom(s2f(X)) || X <- Flags0],
    Form = ?es:record_field(
              es_atom(s2a(Name)),
              ?es:application(
                 ?es:atom(decode_flags),
                 [?es:variable(s2a("M_~s", Name)), ?es:list(Flags)])
             ),
    [Form | gen_decoder_record_assign(More)];
gen_decoder_record_assign([{Name, _Size, {enum, _Enum}} | More]) ->
    %% [io_lib:format("~s = enum_~s(M_~s)", [s2a(Name), s2a(Name), s2a(Name)])];
    Form = ?es:record_field(
              es_atom(s2a(Name)),
              ?es:application(
                 es_atom(s2a("enum_~s", Name)),
                 [?es:variable(s2a("M_~s", Name))])
             ),
    [Form | gen_decoder_record_assign(More)];
gen_decoder_record_assign([{Name, boolean} | More]) ->
    %% [io_lib:format("~s = int2bool(M_~s)", [s2a(Name), s2a(Name)])];
    Form = ?es:record_field(
              es_atom(s2a(Name)),
              ?es:application(
                 ?es:atom("int2bool"),
                 [?es:variable(s2a("M_~s", Name))])
             ),
    [Form | gen_decoder_record_assign(More)];
gen_decoder_record_assign([{Name, Fun} | More]) ->
    %% [io_lib:format("~s = decode_~s(M_~s)", [s2a(Name), Fun, s2a(Name)])];
    Form = ?es:record_field(
              es_atom(s2a(Name)),
              ?es:application(
                 es_atom(io_lib:format("decode_~s", [Fun])),
                 [?es:variable(s2a("M_~s", Name))])
             ),
    [Form | gen_decoder_record_assign(More)];
gen_decoder_record_assign([{Name, Size, {array, Multi}} | More]) when is_list(Multi) ->
    %% [io_lib:format("~s = [X || <<X:~w/bytes>> <= M_~s]", [s2a(Name), Size, s2a(Name)])];
    Form = ?es:record_field(
              es_atom(s2a(Name)),
              ?es:list_comp(
                 ?es:variable("X"),
                 [?es:binary_generator(
                     ?es:binary(
                        [?es:binary_field(
                            ?es:variable("X"),
                            ?es:integer(Size),
                            [?es:atom("bytes")])]),
                     ?es:variable(s2a("M_~s", Name)))
                 ]
                )
             ),
    [Form | gen_decoder_record_assign(More)];
gen_decoder_record_assign([{Name, _Size, {array, Multi}} | More]) ->
    %% [io_lib:format("~s = [X || <<X:~w/bytes>> <= M_~s]", [s2a(Name), Multi, s2a(Name)])];
    Form = ?es:record_field(
              es_atom(s2a(Name)),
              ?es:list_comp(
                 ?es:variable("X"),
                 [?es:binary_generator(
                     ?es:binary(
                        [?es:binary_field(
                            ?es:variable("X"),
                            ?es:integer(Multi),
                            [?es:atom("bytes")])]),
                     ?es:variable(s2a("M_~s", Name)))
                 ]
                )
             ),
    [Form | gen_decoder_record_assign(More)];
gen_decoder_record_assign([{Name, _Size, {type, TypeName}} | More]) ->
    %% [io_lib:format("~s = decode_~s(M_~s)", [s2a(Name), TypeName, s2a(Name)])];
    Form = ?es:record_field(
              es_atom(s2a(Name)),
              ?es:application(
                 es_atom(io_lib:format("decode_~s", [TypeName])),
                 [?es:variable(s2a("M_~s", Name))])
             ),
    [Form | gen_decoder_record_assign(More)];
gen_decoder_record_assign([{Name, _Size, _Type} | More]) ->
    %% [io_lib:format("~s = M_~s", [s2a(Name), s2a(Name)])].
    Form = ?es:record_field(es_atom(s2a(Name)), ?es:variable(s2a("M_~s", Name))),
    [Form | gen_decoder_record_assign(More)].

gen_encoder_record_assign([]) ->
    [];
gen_encoder_record_assign([{Value, _} | More])
  when is_integer(Value); is_atom(Value) ->
    gen_encoder_record_assign(More);
gen_encoder_record_assign([Tuple | More]) ->
    Name = element(1, Tuple),
    Form = ?es:record_field(es_atom(s2a(Name)), ?es:variable(s2a("M_~s", Name))),
    [Form | gen_encoder_record_assign(More)].

gen_encoder_field_assign([]) ->
    [];
gen_encoder_field_assign([{'_', 0} | More]) ->
    gen_encoder_field_assign(More);
gen_encoder_field_assign([{'_', Size} | More]) ->
    %% [io_lib:format("0:~w", [Size])];
    Form = ?es:binary_field(?es:integer(0), ?es:integer(Size), []),
    [Form | gen_encoder_field_assign(More)];
gen_encoder_field_assign([{'1', Size} | More]) ->
    %% [io_lib:format("~w:~w", [(1 bsl Size) - 1, Size])];
    Form = ?es:binary_field(?es:integer((1 bsl Size) - 1), ?es:integer(Size), []),
    [Form | gen_encoder_field_assign(More)];
gen_encoder_field_assign([{Value, Size} | More]) when is_integer(Value) ->
    %% [io_lib:format("~w:~w", [Value, Size])];
    Form = ?es:binary_field(?es:integer(Value), ?es:integer(Size), []),
    [Form | gen_encoder_field_assign(More)];
gen_encoder_field_assign([{Value, Size} | More]) when is_atom(Value) ->
    %% [io_lib:format("~w:~w", [Value, Size])];
    Form = ?es:binary_field(?es:atom(Value), ?es:integer(Size), []),
    [Form | gen_encoder_field_assign(More)];
gen_encoder_field_assign([{Name, {flags, Flags0}} | More]) ->
    %% Flags = [s2f(Flag) || Flag <- reorder_flags(Flags0)],
    %% [io_lib:format("(encode_min_int(8, encode_flags(M_~s, ~p), little))/binary",
    %%                [s2a(Name), Flags])];
    Flags = [es_atom(s2f(Flag)) || Flag <- reorder_flags(Flags0)],
    Form =
        ?es:binary_field(
           ?es:application(
              ?es:atom(encode_min_int),
              [?es:integer(8),
               ?es:application(
                  ?es:atom("encode_flags"),
                  [?es:variable(s2a("M_~s", Name)), ?es:list(Flags)]),
               ?es:atom("little")]),
           [?es:atom("binary")]),
    [Form | gen_encoder_field_assign(More)];
gen_encoder_field_assign([{Name, Size, {enum, _Enum}} | More]) ->
    %% [io_lib:format("(enum_~s(M_~s)):~w/integer", [s2a(Name), s2a(Name), Size])];
    Form =
        ?es:binary_field(
           ?es:application(es_atom(s2a("enum_~s", Name)), [?es:variable(s2a("M_~s", Name))]),
           ?es:integer(Size),
           [?es:atom("integer")]),
    [Form | gen_encoder_field_assign(More)];
gen_encoder_field_assign([{Name, boolean} | More]) ->
    %% [io_lib:format("(bool2int(M_~s)):1", [s2a(Name)])];
    Form =
        ?es:binary_field(
           ?es:application(?es:atom("bool2int"), [?es:variable(s2a("M_~s", Name))]),
           ?es:integer(1),
           []),
    [Form | gen_encoder_field_assign(More)];
gen_encoder_field_assign([{Name, Fun} | More]) ->
    %% [io_lib:format("(encode_~s(M_~s))/binary", [Fun, s2a(Name)])];
    Form =
        ?es:binary_field(
           ?es:application(es_atom(io_lib:format("encode_~s", [Fun])), [?es:variable(s2a("M_~s", Name))]),
           [?es:atom("binary")]),
    [Form | gen_encoder_field_assign(More)];
gen_encoder_field_assign([{Name, Len, {array, _Multi}} | More]) ->
    %% [io_lib:format("(length(M_~s)):~w/integer, (<< <<X/binary>> || X <- M_~s>>)/binary", [s2a(Name), Len, s2a(Name)])];
    Form1 =
        ?es:binary_field(
           ?es:application(?es:atom("length"), [?es:variable(s2a("M_~s", Name))]),
           ?es:integer(Len),
           [?es:atom("integer")]),
    Form2 =
        ?es:binary_field(
           %% (<< <<X/binary>> || X <- M_tripple>>)/binary
           ?es:binary_comp(
              %% <<X/binary>>
              ?es:binary([?es:binary_field(?es:variable("X"), [?es:atom(binary)])]),
              %% X <- M_tripple
              [?es:generator(?es:variable("X"), ?es:variable(s2a("M_~s", Name)))]),
           [?es:atom("binary")]),
    [Form1, Form2 | gen_encoder_field_assign(More)];
gen_encoder_field_assign([{Name, 0, {type, TypeName}} | More]) ->
    %% [io_lib:format("(encode_~s(M_~s))/binary", [TypeName, s2a(Name)])];
    Form =
        ?es:binary_field(
           ?es:application(es_atom(s2a("encode_~s", TypeName)), [?es:variable(s2a("M_~s", Name))]),
           [?es:atom("binary")]),
    [Form | gen_encoder_field_assign(More)];
gen_encoder_field_assign([{Name, Size, {type, TypeName}} | More]) ->
    %% [io_lib:format("(encode_~s(M_~s)):~w/bits", [TypeName, s2a(Name), Size])];
    Form =
        ?es:binary_field(
           ?es:application(es_atom(s2a("encode_~s", TypeName)), [?es:variable(s2a("M_~s", Name))]),
           ?es:integer(Size),
           [?es:atom("bits")]),
    [Form | gen_encoder_field_assign(More)];
gen_encoder_field_assign([{Name, Len, length_binary} | More]) ->
    %% [io_lib:format("(byte_size(M_~s)):~w/integer, M_~s/binary", [s2a(Name), Len, s2a(Name)])];
    Form1 =
        ?es:binary_field(
           ?es:application(?es:atom("byte_size"), [?es:variable(s2a("M_~s", Name))]),
           ?es:integer(Len),
           [?es:atom("integer")]),
    Form2 =
        ?es:binary_field(
           ?es:variable(s2a("M_~s", Name)),
           [?es:atom("binary")]),
    [Form1, Form2 | gen_encoder_field_assign(More)];
gen_encoder_field_assign([{Name, 0, Type} | More]) ->
    %% [io_lib:format("M_~s/~w", [s2a(Name), Type])];
    Form =
        ?es:binary_field(
           ?es:variable(s2a("M_~s", Name)),
           [?es:atom(Type)]),
    [Form | gen_encoder_field_assign(More)];
gen_encoder_field_assign([{Name, Size, bytes} | More]) ->
    %% [io_lib:format("M_~s:~w/bytes", [s2a(Name), Size])];
    Form =
        ?es:binary_field(
           ?es:variable(s2a("M_~s", Name)),
           ?es:integer(Size),
           [?es:atom("bytes")]),
    [Form | gen_encoder_field_assign(More)];
gen_encoder_field_assign([{Name, Size, bits} | More]) ->
    %% [io_lib:format("M_~s:~w/bits", [s2a(Name), Size])];
    Form =
        ?es:binary_field(
           ?es:variable(s2a("M_~s", Name)),
           ?es:integer(Size),
           [?es:atom("bits")]),
    [Form | gen_encoder_field_assign(More)];
gen_encoder_field_assign([{Name, Size, _Type} | More]) ->
    %% [io_lib:format("M_~s:~w", [s2a(Name), Size])].
    Form =
        ?es:binary_field(
           ?es:variable(s2a("M_~s", Name)),
           ?es:integer(Size),
           []),
    [Form | gen_encoder_field_assign(More)].


s2a(Name) when is_atom(Name) ->
    Name;
s2a(Name) ->
    lists:map(fun(32) -> $_;
                 ($/) -> $_;
                 ($-) -> $_;
                 ($.) -> $_;
                 ($,) -> $_;
                 (C)  -> C
              end,
              string:to_lower(Name)).

s2a(Format, Name) ->
    unicode:characters_to_list(io_lib:format(Format, [s2a(Name)])).

s2f(Flags) when is_list(Flags) ->
    list_to_atom(Flags).

gen_enum(Value, Cnt, Next, {FwdFuns, RevFuns}) ->
    Fwd = ?es:clause([es_atom(s2a(Value))], none, [?es:integer(Cnt)]),
    Rev = ?es:clause([?es:integer(Cnt)], none, [es_atom(s2a(Value))]),
    gen_enum(Next, Cnt + 1, {[Fwd|FwdFuns], [Rev|RevFuns]}).

gen_enum([], _, {FwdFuns, RevFuns}) ->
    {lists:reverse(FwdFuns), lists:reverse(RevFuns)};
gen_enum([{Cnt, Value}|Rest], _, Acc) ->
    gen_enum(Value, Cnt, Rest, Acc);
gen_enum([Value|Rest], Cnt, Acc) ->
    gen_enum(Value, Cnt, Rest, Acc).

build_decoder_body([]) ->
    [];
build_decoder_body([H = {_Name, _Len, {array, _Multi}} | Fields]) ->
    build_decoder_body(H, Fields).

build_decoder_body({Name, Len, {array, Multi}}, Fields)
  when is_integer(Len), is_list(Multi) ->
    %% Init = io_lib:format("M_~s_size = M_~s * ~w", [s2a(Name), s2a(Multi), Len]),
    Form = ?es:match_expr(
              ?es:variable(s2a("M_~s_size", Name)),
              ?es:infix_expr(
                 ?es:variable(s2a("M_~s", Multi)),
                 ?es:operator("*"),
                 ?es:integer(Len))),
    [Form | build_decoder_body(Name, Fields)];
build_decoder_body({Name, _Len, {array, Multi}}, Fields)
  when is_integer(Multi) ->
    %% Init = io_lib:format("M_~s_size = M_~s_len * ~w", [s2a(Name), s2a(Name), Multi]),
    Form = ?es:match_expr(
              ?es:variable(s2a("M_~s_size", Name)),
              ?es:infix_expr(
                 ?es:variable(s2a("M_~s_len", Name)),
                 ?es:operator("*"),
                 ?es:integer(Multi))),
    [Form | build_decoder_body(Name, Fields)];

build_decoder_body(Name, Fields) ->
    Next = lists:dropwhile(fun decoder_header_match_split/1, Fields),
    Match = [
             %% io_lib:format("M_~s:M_~s_size/bytes", [s2a(Name), s2a(Name)])
             ?es:binary_field(
                ?es:variable(s2a("M_~s", Name)),
                ?es:variable(s2a("M_~s_size", Name)),
                [?es:atom("bytes")])
            | gen_decoder_header_match(Fields)],
    %% io_lib:format("    <<~s>> = M_~s_Rest,", [string:join(Body, ",\n      "), s2a(Name)]),
    Form = ?es:match_expr(?es:binary(Match), ?es:variable(s2a("M_~s_Rest", Name))),
    [Form | build_decoder_body(Next)].

reorder_flags([]) -> [];
reorder_flags(Flags) ->
    {Head, Tail} = lists:split(8, Flags),
    lists:reverse(Head) ++ reorder_flags(Tail).

collect_enum({Name, _, {enum, Enum}}, Acc) ->
    {FwdFuns, RevFuns} = gen_enum(Enum, 0, {[], []}),

    Wildcard = ?es:clause([?es:variable("X")],
                          [?es:application(?es:atom(is_integer), [?es:variable("X")])],
                          [?es:variable("X")]),
    Fun =
        ?es:function(
           es_atom(io_lib:format("enum_~s", [s2a(Name)])),
           FwdFuns ++ RevFuns ++ [Wildcard]),
    lists:keystore(Name, 1, Acc, {Name, Fun});
collect_enum(_, Acc) ->
    Acc.

collect_enums({_, _, _, Fields}, AccIn)
  when is_list(Fields) ->
    lists:foldr(fun(X, Acc) -> collect_enum(X, Acc) end, AccIn, Fields);
collect_enums(_, AccIn) ->
    AccIn.

write_enums(IEs) ->
    {_, Enums} = lists:unzip(lists:foldr(fun(X, Acc) -> collect_enums(X, Acc) end, [], IEs)),
    Enums.

write_record(Name, FieldDefs) ->
    ?es:attribute(?es:atom(record), [es_atom(s2a(Name)), ?es:tuple(FieldDefs)]).

write_record([]) ->
    [];
write_record([{_Id, Name, Length, []} | Next])
  when is_integer(Length) ->
    FieldDefs =
        gen_record_def([{"Instance", 0, integer},
                        {"Content", Length, bytes}]),
    [write_record(Name, FieldDefs) | write_record(Next)];
write_record([{_Id, Name, _Length, Fields} | Next])
  when is_list(Fields) ->
    FieldDefs = gen_record_def([{"Instance", 0, integer} | Fields]),
    [write_record(Name, FieldDefs) | write_record(Next)];
write_record([_|Next]) ->
    write_record(Next).

write_decoder({Id, Name, _Length, Fields})
  when is_list(Fields) ->
    Match = gen_decoder_header_match(Fields),
    RecordAssigns = gen_decoder_record_assign(Fields),
    BodyFields = lists:dropwhile(fun decoder_header_match_split/1, Fields),
    Body = build_decoder_body(BodyFields),
    ?es:clause(
       [?es:binary(Match), ?es:integer(Id), ?es:variable("Instance")],
       none,
       Body ++ [?es:record_expr(
                   es_atom(s2a(Name)),
                   [?es:record_field(?es:atom("instance"), ?es:variable("Instance"))
                   | RecordAssigns])
               ]);
write_decoder({Id, _Name, Helper})
  when is_atom(Helper) ->
    %% io_lib:format("decode_v1_element(<<Data/binary>>, ~w, Instance) ->~n    decode_~s(Data, Instance)",
    %%               [Id, Helper]);
    ?es:clause(
       [?es:binary(
           [?es:binary_field(
               ?es:variable("Data"),
               [?es:atom("binary")])]),
        ?es:integer(Id), ?es:variable("Instance")],
       none,
       [?es:application(
           es_atom(io_lib:format("decode_~s", [Helper])),
           [?es:variable("Data"), ?es:variable("Instance")])
       ]);
write_decoder({Id, _Name, Length, Helper})
  when is_integer(Length), is_atom(Helper) ->
    %% io_lib:format("decode_v1_element(<<Data/binary>>, ~w, Instance) ->~n    decode_~s(Data, Instance)",
    %%               [Id, Helper]).
    ?es:clause(
       [?es:binary(
           [?es:binary_field(
               ?es:variable("Data"),
               [?es:atom("binary")])]),
        ?es:integer(Id), ?es:variable("Instance")],
       none,
       [?es:application(
           es_atom(io_lib:format("decode_~s", [Helper])),
           [?es:variable("Data"), ?es:variable("Instance")])
       ]).

write_rec_encoder(Id, Name, RecordAssigns, FieldAssigns) ->
    ?es:clause(
       [?es:record_expr(
           es_atom(s2a(Name)),
           [?es:record_field(
               ?es:atom("instance"), ?es:variable("Instance")) | RecordAssigns])
       ],
       none,
       [?es:application(
           ?es:atom(encode_v1_element),
           [?es:integer(Id), ?es:variable("Instance"), ?es:binary(FieldAssigns)])
       ]).

write_encoder({Id, Name, Length, []})
  when is_integer(Length) ->
    erlang:error(exit),
    RecordAssigns = ["content = Content"],
    FieldAssigns = [io_lib:format("Content:~w/bytes", [Length])],
    write_rec_encoder(Id, Name, RecordAssigns, FieldAssigns);
write_encoder({Id, Name, _Length, Fields})
  when is_list(Fields) ->
    RecordAssigns = gen_encoder_record_assign(Fields),
    FieldAssigns = gen_encoder_field_assign(Fields),
    write_rec_encoder(Id, Name, RecordAssigns, FieldAssigns);
write_encoder({Id, Name, Helper})
  when is_atom(Helper) ->
    %% io_lib:format("encode_v1_element(#~s{instance = Instance} = IE) ->~n    ~s(~w, Instance, encode_~s(IE))",
    %%               [s2a(Name), "FunName", Id, Helper]);
    ?es:clause(
       [?es:match_expr(
           ?es:record_expr(
              es_atom(s2a(Name)),
              [?es:match_expr(?es:atom("instance"), ?es:variable("Instance"))]),
           ?es:variable("IE"))
       ],
       none,
       [?es:application(
           ?es:atom(encode_v1_element),
           [?es:integer(Id), ?es:variable("Instance"),
            ?es:application(
               es_atom(s2a("encode_~s", Helper)),
               [?es:variable("IE")])
           ])
       ]);
write_encoder({Id, Name, Length, Helper})
  when is_integer(Length), is_atom(Helper) ->
    %% io_lib:format("encode_v1_element(#~s{instance = Instance} = IE) ->~n    ~s(~w, Instance, encode_~s(IE))",
    %%               [s2a(Name), "FunName", Id, Helper]).
    ?es:clause(
       [?es:match_expr(
           ?es:record_expr(
              es_atom(s2a(Name)),
              [?es:match_expr(?es:atom("instance"), ?es:variable("Instance"))]),
           ?es:variable("IE"))
       ],
       none,
       [?es:application(
           ?es:atom(encode_v1_element),
           [?es:integer(Id), ?es:variable("Instance"),
            ?es:application(
               es_atom(s2a("encode_~s", Helper)),
               [?es:variable("IE")])
           ])
       ]).

%% development helper and code snippets, keep them
%%
%% to_form(IoList) ->
%%     String = unicode:characters_to_list(IoList),
%%     {ok, Tokens, _} = erl_scan:string(String),
%%     {ok, Forms} = erl_parse:parse_form(Tokens),
%%     Forms.
%%
%% io:format("GeneratedForm:\n~p\n", [Form]),
%% io:format("Form:\n~p\n", [erl_syntax:revert(Form)]),
%% io:format("Text:\n~s\n", [erl_prettypr:format(Form)]),

es_atom(List) when is_list(List) ->
    es_atom(iolist_to_binary(List));
es_atom(Bin) when is_binary(Bin) ->
    ?es:atom(binary_to_list(Bin));
es_atom(Atom) when is_atom(Atom) ->
    ?es:atom(Atom).

msg_description(Msgs) ->
    MsgDescriptionClauses =
        [?es:clause([es_atom(s2a(X))], none, [?es:binary([?es:string(X)])]) || {_, X} <- Msgs],
    MsgDescriptionCatchAllClause =
        ?es:clause(
           [?es:variable("X")], none,
           [?es:application(?es:atom(io_lib), ?es:atom(format),
                            [?es:string("~p"), ?es:list([?es:variable("X")])])]),

    ?es:function(?es:atom(msg_description),
                 MsgDescriptionClauses ++ [MsgDescriptionCatchAllClause]).

message_type(Msgs) ->
    FwdFuns = [?es:clause([es_atom(s2a(Name))], none, [?es:integer(Value)]) ||
                  {Value, Name} <- Msgs],
    RevFuns = [?es:clause([?es:integer(Value)], none, [es_atom(s2a(Name))]) ||
                  {Value, Name} <- Msgs],
    ErrorFun = ?es:clause(
                  [?es:variable("Type")], none,
                  [?es:application(?es:atom(error),
                                   [?es:atom(badarg), ?es:list([?es:variable("Type")])])]),
    Clauses = FwdFuns ++ RevFuns ++ [ErrorFun],
    ?es:function(?es:atom(message_type_v1), Clauses).

ie_type_macros(IEs) ->
    [?es:attribute(
        ?es:atom(define),
        [?es:text(string:uppercase(s2a("GTP_V1_IE_~s", element(2, IE)))), ?es:integer(element(1, IE))])
     || IE <- IEs].

decode_v1_element(IEs) ->
    DecoderClauses = [write_decoder(X) || X <- IEs],
    DecoderCatchAny =
        ?es:clause(
           [%% decode_v1_element(Value, Tag, Instance)
            ?es:variable("Value"), ?es:variable("Tag"), ?es:variable("Instance")],
           none,
           %% {Tag, Instance, Value}
           [?es:tuple([?es:variable("Tag"), ?es:variable("Instance"), ?es:variable("Value")])]
          ),
    ?es:function(
       ?es:atom(decode_v1_element),
       DecoderClauses ++ [DecoderCatchAny]).

decode_v1_ie(IEs) ->
    EmptyClause =
        ?es:clause(
           [?es:binary([]), ?es:variable("_Wanted"), ?es:variable("_PrevId"), ?es:variable("_PrevInst"),
            ?es:variable("IEs")],
           none,
           [?es:variable("IEs")]),
    Clauses =
        [?es:clause(
            [?es:binary(
                [?es:binary_field(?es:integer(Id)),
                 ?es:binary_field(
                    ?es:variable("Data"),
                    [?es:atom("binary")])
                ]),
             ?es:variable("Wanted"),
             ?es:variable("PrevId"),
             ?es:variable("PrevInst"),
             ?es:variable("IEs")
            ],
            none,
            [?es:application(
                ?es:atom("decode_v1_ie"),
                [?es:variable("Data"),
                 ?es:integer(Id),
                 ?es:integer(Length),
                 ?es:variable("Wanted"),
                 ?es:variable("PrevId"),
                 ?es:variable("PrevInst"),
                 ?es:variable("IEs")
                ])
            ])
         || {Id, _, Length, _} <- IEs, Id < 128],

    FinalClauses =
        [
         %% decode_v1(<<Id, Length:16/integer, Data/binary>>, Wanted, PrevId, PrevInst, IEs) when Id > 127 ->
         ?es:clause(
            [?es:binary(
                [?es:binary_field(?es:variable("Id")),
                 ?es:binary_field(
                    ?es:variable("Length"),
                    ?es:integer(16),
                    [?es:atom("integer")]),
                 ?es:binary_field(
                    ?es:variable("Data"),
                    [?es:atom("binary")])
                ]),
             ?es:variable("Wanted"),
             ?es:variable("PrevId"),
             ?es:variable("PrevInst"),
             ?es:variable("IEs")
            ],
            [?es:infix_expr(?es:variable("Id"), ?es:operator(">"), ?es:integer(127))],
            [?es:application(
                ?es:atom("decode_v1_ie"),
                [?es:variable("Data"),
                 ?es:variable("Id"),
                 ?es:variable("Length"),
                 ?es:variable("Wanted"),
                 ?es:variable("PrevId"),
                 ?es:variable("PrevInst"),
                 ?es:variable("IEs")
                ])
            ]),
         %% decode_v1(<<Id, Data/binary>>, Wanted, PrevId, PrevInst, IEs) ->
         ?es:clause(
            [?es:binary(
                [?es:binary_field(?es:variable("Id")),
                 ?es:binary_field(
                    ?es:variable("Rest"),
                    [?es:atom("binary")])
                ]),
             ?es:variable("_Wanted"),
             ?es:variable("_PrevId"),
             ?es:variable("_PrevInst"),
             ?es:variable("_IEs")
            ],
            none,
            [?es:application(
                ?es:atom(error),
                [?es:atom(badarg), ?es:list([?es:variable("Id"), ?es:variable("Rest")])])
            ])
        ],
    ?es:function(
       ?es:atom(decode_v1_ie),
       [EmptyClause] ++ Clauses ++ FinalClauses).

decode_v1(IEs) ->
    MainDecodeSwitchEmptyClause =
        ?es:clause(
           [?es:binary([]), ?es:variable("_PrevId"), ?es:variable("_PrevInst"),
            ?es:variable("IEs")],
           none,
           [?es:variable("IEs")]),
    MainDecodeSwitchClauses =
        [?es:clause(
            [?es:binary(
                [?es:binary_field(?es:integer(Id)),
                 ?es:binary_field(
                    ?es:variable("Data"),
                    ?es:integer(Length),
                    [?es:atom("bytes")]),
                 ?es:binary_field(
                    ?es:variable("Next"),
                    [?es:atom("binary")])
                ]),
             ?es:variable("PrevId"),
             ?es:variable("PrevInst"),
             ?es:variable("IEs")
            ],
            none,
            [?es:match_expr(
                ?es:variable("Instance"),
                ?es:application(
                   ?es:atom(v1_instance),
                   [?es:integer(Id), ?es:variable("PrevId"), ?es:variable("PrevInst")])),
             ?es:match_expr(
                ?es:variable("IE"),
                ?es:application(
                   ?es:atom(decode_v1_element),
                   [?es:variable("Data"), ?es:integer(Id), ?es:variable("Instance")])),
             ?es:application(
                ?es:atom(decode_v1),
                [?es:variable("Next"), ?es:integer(Id), ?es:variable("Instance"),
                 ?es:application(?es:atom(put_ie), [?es:variable("IE"), ?es:variable("IEs")])
                ])
            ])
         || {Id, _, Length, _} <- IEs, Id < 128],

    MainDecodeFinalClauses =
        [
         %% decode_v1(<<Id, Length:16/integer, Rest/binary>>, PrevId, PrevInst, IEs) when Id > 127 ->
         ?es:clause(
            [?es:binary(
                [?es:binary_field(?es:variable("Id")),
                 ?es:binary_field(
                    ?es:variable("Length"),
                    ?es:integer(16),
                    [?es:atom("integer")]),
                 ?es:binary_field(
                    ?es:variable("Rest"),
                    [?es:atom("binary")])
                ]),
             ?es:variable("PrevId"),
             ?es:variable("PrevInst"),
             ?es:variable("IEs")
            ],
            [?es:infix_expr(?es:variable("Id"), ?es:operator(">"), ?es:integer(127))],
            [
             %% <<Data:Length/binary, Next/binary>> = Rest,
             ?es:match_expr(
                ?es:binary(
                   [?es:binary_field(
                       ?es:variable("Data"),
                       ?es:variable("Length"),
                       [?es:atom("bytes")]),
                    ?es:binary_field(
                       ?es:variable("Next"),
                       [?es:atom("binary")])
                   ]),
                ?es:variable("Rest")),
             %% Instance = v1_instance(Id, PrevId, PrevInst)
             ?es:match_expr(
                ?es:variable("Instance"),
                ?es:application(
                   ?es:atom(v1_instance),
                   [?es:variable("Id"), ?es:variable("PrevId"), ?es:variable("PrevInst")])),
             %% IE = decode_v1_element(Data, Id, Instance)
             ?es:match_expr(
                ?es:variable("IE"),
                ?es:application(
                   ?es:atom(decode_v1_element),
                   [?es:variable("Data"), ?es:variable("Id"), ?es:variable("Instance")])),
             %% decode_v1(Next, Id, Instance, put_ie(IE, IEs))
             ?es:application(
                ?es:atom(decode_v1),
                [?es:variable("Next"), ?es:variable("Id"), ?es:variable("Instance"),
                 ?es:application(?es:atom(put_ie), [?es:variable("IE"), ?es:variable("IEs")])
                ])
            ]),
         %% decode_v1(<<Id, Rest/binary>>, PrevId, PrevInst, IEs) ->
         ?es:clause(
            [?es:binary(
                [?es:binary_field(?es:variable("Id")),
                 ?es:binary_field(
                    ?es:variable("Rest"),
                    [?es:atom("binary")])
                ]),
             ?es:variable("PrevId"),
             ?es:variable("PrevInst"),
             ?es:variable("IEs")
            ],
            none,
            [%% Instance = v1_instance(Id, PrevId, PrevInst)
             ?es:match_expr(
                ?es:variable("Instance"),
                ?es:application(
                   ?es:atom(v1_instance),
                   [?es:variable("Id"), ?es:variable("PrevId"), ?es:variable("PrevInst")])),
             %% IE = {Id, Instance, Rest}
             ?es:match_expr(
                ?es:variable("IE"),
                ?es:tuple(
                   [?es:variable("Id"), ?es:variable("Instance"), ?es:variable("Rest")])),
             %% decode_v1(<<>>, Id, Instance, put_ie(IE, IEs))
             ?es:application(
                ?es:atom(decode_v1),
                [?es:binary([]), ?es:variable("Id"), ?es:variable("Instance"),
                 ?es:application(?es:atom(put_ie), [?es:variable("IE"), ?es:variable("IEs")])])
            ])
        ],
    ?es:function(
       ?es:atom(decode_v1),
       [MainDecodeSwitchEmptyClause] ++ MainDecodeSwitchClauses ++ MainDecodeFinalClauses).

encode_v1_element(IEs) ->
    EncoderClauses = [write_encoder(X) || X <- IEs],
    EncoderCatchAny =
        ?es:clause(
           [%% encode_v1_element({Tag, Instance, Value})
            ?es:tuple(
               [?es:variable("Tag"), ?es:variable("Instance"), ?es:variable("Value")])],
           %% when is_integer(Tag), is_integer(Instance), is_binary(Value)
           [?es:application(?es:atom("is_integer"), [?es:variable("Tag")]),
            ?es:application(?es:atom("is_integer"), [?es:variable("Instance")]),
            ?es:application(?es:atom("is_binary"), [?es:variable("Value")])],
           %% encode_v1_element(Tag, Instance, Value)
           [?es:application(?es:atom("encode_v1_element"),
                            [?es:variable("Tag"), ?es:variable("Instance"), ?es:variable("Value")])]
          ),
    ?es:function(
       ?es:atom(encode_v1_element),
       EncoderClauses ++ [EncoderCatchAny]).

main(_) ->
    IEs = ies(),

    IETypeMacros = ie_type_macros(IEs),
    Records = write_record(IEs),
    ExpRecs = ?es:attribute(
                 ?es:atom(define),
                 [?es:text("GTP_V1_RECORDS"),
                  ?es:list([es_atom(s2a(element(2, ExpRec))) || ExpRec <- IEs])]),
    HrlForms = ?es:form_list(
                  IETypeMacros ++
                      [ExpRecs | Records] ++ [?es:eof_marker()]),
    HrlRecs = erl_prettypr:format(HrlForms),

    MsgDescription = msg_description(msgs()),
    MessageTypes = message_type(msgs()),
    Enums = write_enums(IEs),
    Decoder = decode_v1_element(IEs),
    DecodeIE = decode_v1_ie(IEs),
    MainDecodeSwitch = decode_v1(IEs),
    Encoder = encode_v1_element(IEs),

    ErlForms =
        ?es:form_list(
           [MsgDescription, MessageTypes] ++ Enums ++
               [Decoder, DecodeIE, MainDecodeSwitch, Encoder, ?es:eof_marker()]),
    ErlDecls = erl_prettypr:format(ErlForms),

    {ok, HrlF0} = file:read_file("include/gtp_packet.hrl"),
    [HrlHead, _, HrlV2] = binary:split(HrlF0, [?V1_TAG, ?V2_TAG], [global]),
    file:write_file("include/gtp_packet.hrl", [HrlHead, ?V1_TAG, "\n\n", HrlRecs, ?V2_TAG, HrlV2]),

    {ok, ErlF0} = file:read_file("src/gtp_packet.erl"),
    [ErlHead, _, ErlV2] = binary:split(ErlF0, [?V1_TAG, ?V2_TAG], [global]),
    file:write_file("src/gtp_packet.erl", [ErlHead, ?V1_TAG, "\n\n", ErlDecls, ?V2_TAG, ErlV2]).
